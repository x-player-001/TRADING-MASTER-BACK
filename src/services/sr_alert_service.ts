...
export class SRAlertService {
  ...
  async process(
    ...
    // 1. 更新支撑阻力位
    const levels = this.update_levels(symbol, interval, klines);

    // 2. 检查报警
    const alerts = this.check_alerts(symbol, interval, current_price, kline_time);
      ...
      for (const alert of alerts) {
        try {
          ...
          if (!exists) {
            ...
            logger.info(`SR Alert: ${alert.description}`);
          }
        } catch (error) {
