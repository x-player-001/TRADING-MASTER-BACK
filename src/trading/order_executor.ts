...
export class OrderExecutor {
  ...
  async get_klines(
    ...
    if (!this.binance_api) {
      logger.warn('[OrderExecutor] Binance API not initialized for getting klines');
      return null;
    }

    try {
      // 调用币安API获取K线（使用正确的参数签名）
      ...
      logger.error(`[OrderExecutor] Failed to get klines for ${symbol}:`, error);
      return null;
    }
  }
}
