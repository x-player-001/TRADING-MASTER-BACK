...
export class BinanceFuturesTradingAPI {
  ...
  async cancel_algo_order(algoId: number): Promise<{ success: boolean; algoId: number }> {
    try {
      ...
      const response = await this.api_client.delete('/fapi/v1/algoOrder', {
        params: { ...params, signature }
      });
   ...
   * 端点: DELETE /fapi/v1/algoOpenOrders
   * @param symbol 交易对
   */
  async cancel_all_algo_orders(symbol: string): Promise<{ success: boolean }> {
    try {
      const timestamp = Date.now();
      const params = {
        symbol,
      ...
      await this.api_client.delete('/fapi/v1/algoOpenOrders', {
        params: { ...params, signature }
      });

      logger.info(`[BinanceTradingAPI] All algo orders cancelled for ${symbol}`);
      return { success: true };
    } catch (error: any) {
      const error_code = error.response?.data?.code?.toString();
      const error_msg = error.response?.data?.msg || error.message;
      logger.error(`[BinanceTradingAPI] Failed to cancel all algo orders for ${symbol}:`, error.response?.data || error.message);
      errorLogRepository.log_order_error(
        `Failed to cancel all algo orders: ${error_msg}`,
        symbol,
        { error_data: error.response?.data },
        error_code
      );
