 ...
 * 配置说明:
 * - 初始资金: $50
 * - 追高阈值: 20% (price_from_low_pct > 20% 拒绝)
 * - 其他配置与标准配置相同
 */
...
async function run_backtest() {
  console.log('🚀 启动 $50 小资金配置回测 - 追高阈值20%\\n');
  ...
  try {
    ...
    const json_file = path.join(results_dir, `backtest_threshold_20_${timestamp}.json`);
