 ...
 * 配置:
 * - 初始资金: $1000
 * - 回测周期: 2025-11-21 00:00 ~ 2025-11-25 23:59
 * - 信号过滤: 评分 ≥ 5分
 * - 开仓金额: 固定 $50/笔
 * - 超时平仓: 12小时
...
async function run_backtest() {
  console.log('🚀 开始回测测试 (11月21日之后 - 逐仓模式)\n');
  ...
  try {
    ...
    // 设置时间范围：11月21日00:00 到 11月25日23:59
    const start_date = new Date('2025-11-21T00:00:00Z');
    const end_date = new Date('2025-11-25T23:59:59Z');
    ...
    const json_file = path.join(results_dir, `backtest_nov21_after_${timestamp}.json`);
    fs.writeFileSync(json_file, JSON.stringify(full_result, null, 2));
    console.log(`\n💾 交易明细已保存: ${json_file}`);

    // 保存简化版文本摘要
    const summary_file = path.join(results_dir, `backtest_nov21_after_${timestamp}.txt`);
    const summary_text = `
11月21日后回测结果摘要
═══════════════════════════════════════════════════════════════════════════════
