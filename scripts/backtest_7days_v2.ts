...
async function run_backtest() {
  ...
  try {
    ...
    const json_file = path.join(results_dir, `backtest_7days_${timestamp}.json`);
    fs.writeFileSync(json_file, JSON.stringify(full_result, null, 2));
    console.log(`\n💾 交易明细已保存: ${json_file}`);

    // 保存简化版文本摘要
    const summary_file = path.join(results_dir, `backtest_7days_${timestamp}.txt`);
    const summary_text = `
7天回测结果摘要
═══════════════════════════════════════════════════════════════════════════════
