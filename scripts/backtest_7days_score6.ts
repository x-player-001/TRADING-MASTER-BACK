...
async function run_backtest() {
  console.log('🚀 开始7天回测测试 (逐仓模式 - 评分≥6分)\n');
  ...
  try {
    ...
    const trade_details = backtest_result.trades.map(trade => ({
      ...
      signal_score: trade.signal_id // 假设signal_id存储了评分信息
    }));
    ...
    const json_file = path.join(results_dir, `backtest_7days_score6_${timestamp}.json`);
    fs.writeFileSync(json_file, JSON.stringify(full_result, null, 2));
    console.log(`\n💾 交易明细已保存: ${json_file}`);

    // 保存简化版文本摘要
    const summary_file = path.join(results_dir, `backtest_7days_score6_${timestamp}.txt`);
    const summary_text = `
7天回测结果摘要
═══════════════════════════════════════════════════════════════════════════════
